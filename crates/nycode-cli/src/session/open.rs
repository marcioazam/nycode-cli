...
fn open(store: &Store, cli: &Cli) -> anyhow::Result<(String, Vec<Message>)> {
    ...
    if let Some(id) = &cli.session_id {
        validate_id(id)?;
        return if store.checked_path_for(id)?.is_file() {
...
fn fork_src(store: &Store, src: &str) -> anyhow::Result<(PathBuf, CopyKind)> {
    ...
    let as_session =
        validate_id(src).is_ok() && store.checked_path_for(src).is_ok_and(|path| path.is_file());
    if as_session {
        return Ok((store.checked_path_for(src)?, CopyKind::Session));
    }
    if path.is_file() {
    ...
    anyhow::bail!(
        ...
        store.checked_path_for(src)?.display()
    )
}
...
fn copy_into(
    ...
    let dest_path = store.checked_path_for(&id)?;
...
mod tests {
    ...
    fn import_copies_a_jsonl_file_into_the_store() {
        ...
        std::fs::copy(store.checked_path_for("origem").unwrap(), &exported).unwrap();
        let (_id, history) =
    ...
    fn fork_of_a_cwd_file_named_like_an_id_copies_the_file_not_the_store() {
        ...
        std::fs::copy(store.checked_path_for("origem").unwrap(), &path).unwrap();
