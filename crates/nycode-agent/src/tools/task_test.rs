...
use super::*;
use crate::agent_test::{text_turn, tool_turn};
...
async fn child_output(backend: Arc<FakeBackend>, ctx: &ToolContext) -> ToolOutput {
    let task = Task::new(backend).await.unwrap();
    run(&task, "faca algo", ctx).await
}
...
async fn a_child_that_answers_nothing_is_reported_as_a_failure() {
    ...
    // tivesse a resposta.
    let (_dir, ctx) = workspace();
    let out = child_output(Arc::new(FakeBackend::new(vec![text_turn("   ")])), &ctx).await;
...
async fn a_child_that_fails_says_so_instead_of_answering_empty() {
    ...
    let out = child_output(
        Arc::new(FakeBackend::failing(nycode_ai::Error::TruncatedStream {
            bytes: 3,
        })),
        &ctx,
    )
