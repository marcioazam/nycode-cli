...
impl Confinement {
    ...
    /// Aviso a mostrar quando não há confinamento.
    ///
    /// Rodar sem sandbox em silêncio é a degradação que o NFR-4 proíbe: a
    ...
    pub fn warning(&self) -> Option<String> {
        match self {
            ...
            Self::Bubblewrap { .. } | Self::Seatbelt { .. } => None,
        }
    }
}
...
pub use profile::{prefix, wrap, Policy};
...
mod tests {
    ...
    #[test]
    fn an_enforced_confinement_does_not_warn() {
        assert!(detect_on(Platform::Linux, &everything_available)
            .warning()
            .is_none());
        assert!(detect_on(Platform::MacOs, &everything_available)
            .warning()
            .is_none());
    }
