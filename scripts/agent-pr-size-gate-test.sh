...
commit "${box}" f.txt 800 "Claude Code:claude-sonnet-5"
check 0 "800 linhas alteradas passa; o teto e inclusivo" "${box}"

box="$(repo acima_do_teto_de_linhas)"
commit "${box}" f.txt 801 "Claude Code:claude-sonnet-5"
check 1 "801 linhas alteradas reprova" "${box}" main feature "linhas"

# --- Arquivos ------------------------------------------------------------------

box="$(repo vinte_e_cinco_arquivos)"
(
	cd "${box}"
...
check 0 "vinte e cinco arquivos passa; o teto e inclusivo" "${box}"

box="$(repo vinte_e_seis_arquivos)"
(
	cd "${box}"
...
check 1 "vinte e seis arquivos reprova" "${box}" main feature "arquivos"
