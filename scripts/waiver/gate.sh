#!/usr/bin/env bash
# Gate de waiver e quarentena expirados (GATE-14 / CI-10 / CI-14).
#
...
#
# `Waiver:` no ADR pode listar varios IDs separados por virgula; cada um
# precisa de linha no registro apontando para o mesmo ADR.
#
# Quarentena de flake: scripts/flake-quarantine.txt, mesmos seis campos,
# mesmo tratamento de data.
#
# Falha (exit 1) quando a data passou, falta campo, o ADR apontado nao
# existe, o cabecalho do ADR diverge do registro, um ADR declara Waiver
# sem linha correspondente, ou um ID da lista do ADR nao tem linha.
#
# Uso:
#   scripts/waiver/gate.sh
#   scripts/waiver/gate.sh <raiz> <registro> [<quarentena>]
...
QUARENTENA="${3:-${TARGET}/scripts/flake-quarantine.txt}"
...
# token_in_list <agulha> <lista-separada-por-virgula>
token_in_list() {
...
while IFS= read -r linha || [[ -n "${linha}" ]]; do
  ...
  if ! token_in_list "${regra}" "${waiver_adr}"; then
...
if [[ -d "${decisoes}" ]]; then
  while IFS= read -r adr_path; do
    ...
    waiver_adr="$(sed -n 's/^- \*\*Waiver:\*\* //p' "${adr_path}" | head -n 1)"
    waiver_adr="$(trim "${waiver_adr}")"
    IFS=',' read -ra toks <<<"${waiver_adr}"
    for tok in "${toks[@]}"; do
      tok="$(trim "${tok}")"
      [[ -z "${tok}" ]] && continue
      par="${rel}"$'\t'"${tok}"
      if [[ -z "${visto_par[${par}]+x}" ]]; then
        echo "  FALHA: ${rel} lista ${tok} em Waiver sem linha no registro" >&2
        failures=$((failures + 1))
      fi
    done
...
if [[ -f "${QUARENTENA}" ]]; then
  while IFS= read -r linha || [[ -n "${linha}" ]]; do
    [[ -z "${linha}" || "${linha}" == \#* ]] && continue

    IFS='|' read -r id teste expira dono controle razao _extra <<<"${linha}"
    id="$(trim "${id}")"
    teste="$(trim "${teste}")"
    expira="$(trim "${expira}")"
    dono="$(trim "${dono}")"
    ...
    if [[ -z "${id}" || -z "${teste}" || -z "${expira}" || -z "${dono}" || -z "${controle}" || -z "${razao}" ]]; then
      echo "  FALHA: quarentena sem os seis campos: ${linha}" >&2
      failures=$((failures + 1))
      continue
    fi
    if [[ ! "${expira}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
      echo "  FALHA: quarentena ${id}: data invalida (${expira}); use YYYY-MM-DD" >&2
      failures=$((failures + 1))
      continue
    fi
    if [[ "${expira}" < "${hoje}" ]]; then
      echo "  FALHA: quarentena ${id} (${teste}) expirou em ${expira}" >&2
      failures=$((failures + 1))
    fi
  done <"${QUARENTENA}"
fi
