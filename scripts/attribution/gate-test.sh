...
sandbox_git() {
	local box="$1"
	shift
	# O worktree que chama este teste pode ter GIT_DIR exportado.
	env -u GIT_DIR -u GIT_WORK_TREE git -C "${box}" "$@"
}

run_gate() {
	local box="$1"
	shift
	env -u GIT_DIR -u GIT_WORK_TREE bash "${box}/scripts/attribution/gate.sh" "$@" 2>&1
}

setup() {
	local box="$1"
	mkdir -p "${box}"
	sandbox_git "${box}" init --quiet
	sandbox_git "${box}" config user.email "dev@example.com"
	sandbox_git "${box}" config user.name "Dev"
	printf 'a\n' >"${box}/f.txt"
	sandbox_git "${box}" add f.txt
	sandbox_git "${box}" commit --quiet -m "base"
	sandbox_gate "${box}"
}
...
printf 'b\n' >>"${box}/f.txt"
sandbox_git "${box}" add f.txt
sandbox_git "${box}" commit --quiet -m "feat: so humano"
...
printf 'b\n' >>"${box}/f.txt"
sandbox_git "${box}" add f.txt
sandbox_git "${box}" commit --quiet -m "$(printf 'feat: x\n\nCo-Authored-By: Grok <grok@x.ai>\n')"
output="$(run_gate "${box}" "HEAD~1" "HEAD")" && status=0 || status=$?
