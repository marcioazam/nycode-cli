...
| Rug pull de servidor MCP | Servidor MCP muda o conjunto de ferramentas depois do consentimento inicial | Revalidação por hash da definição consentida ([ADR-0028](architecture/decisions/0028-o-consentimento-fixa-a-definicao-declarada.md)) | `policy::definition` (`pin_grants_the_first_snapshot_and_refuses_a_silent_change`, `cover_is_order_independent_and_injective`) |
| Injeção de prompt via instrução do projeto | `AGENTS.md`/`SKILL.md`/regra de projeto do repositório carrega instrução maliciosa | Conteúdo do workspace é descoberto, mas não entra no system prompt sem `--trust-workspace-instructions`; contenção de caminho continua ativa (FR-8, FR-10, `AGT-01`) | `context::tests`, `invocation::prompt::tests`, checklist "Postura de agente" |
