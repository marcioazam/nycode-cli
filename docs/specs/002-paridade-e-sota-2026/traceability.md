...
| B27 — sessão nomeada, id escolhido, fork e import | fechado | [`open.rs`](../../../crates/nycode-cli/src/session/open.rs); `--name`, `--session-id`, `--fork`, `--import`. Testes `a_chosen_id_creates_the_session_when_it_is_missing`, `fork_copies_history_into_a_new_id`, `import_copies_a_jsonl_file_into_the_store` |
| B28 — estatísticas, cópia, sessão nova, recarga | fechado | `/session` `/copy` `/new` `/reload` em [`builtin.rs`](../../../crates/nycode-cli/src/interactive/builtin.rs). Testes `session_lists_id_name_and_message_count`, `copy_shows_the_last_assistant_text`, `a_new_session_drops_the_previous_history`, `reload_picks_up_a_command_written_after_startup` |
