...
| A4 — compactação só no erro, retenção por contagem | parcial | gatilho por limiar em [`occupancy.rs`](../../../crates/nycode-agent/src/agent/occupancy.rs) / `next_turn`; o corte ainda retém N mensagens. Teste `a_session_over_the_threshold_compacts_before_the_request` |
| C4 — gatilho por limiar com reserva | fechado | `over_threshold`: ocupação > janela − 16384 tokens. Sem janela, não dispara. Teste `the_threshold_needs_a_declared_window` |
| B9 — estimativa ancorada no último usage | fechado | `occupancy(messages, last)` soma o usage real e estima só a cauda; usage estimado não vira âncora (`note_usage`). Teste `occupancy_uses_the_last_usage_and_only_estimates_the_tail` |
| B10 — resumo com seções nomeadas | fechado | [`SUMMARY_PROMPT`](../../../crates/nycode-agent/src/session/compaction/marker.rs) pede as seis seções nesta ordem; [`summarize_dropped`](../../../crates/nycode-agent/src/agent/shrink.rs) é o chamador. Teste `the_prompt_names_the_sections_in_a_stable_order` |
| B11 — marcador autocontido | fechado | `marker::build` leva a cauda retida; [`chain_to`](../../../crates/nycode-agent/src/session/store/tree.rs) para no marcador de compactação. Testes `the_marker_carries_the_kept_tail`, `the_marker_carries_the_messages_that_were_kept`, `reconstruction_stops_at_a_compaction_marker` |
| B12 — sumarização de ramo abandonado | fechado | [`branch.rs`](../../../crates/nycode-agent/src/session/compaction/branch.rs) `abandoned`/`notice`; `/fork` em [`interactive.rs`](../../../crates/nycode-cli/src/interactive.rs) grava o aviso como filho do ponto escolhido. O aviso não para a reconstrução — o prefixo compartilhado continua. Testes `a_prefix_fork_drops_only_the_suffix`, `a_turn_after_a_fork_hangs_off_the_point_that_was_chosen`, `a_branch_notice_does_not_drop_the_shared_prefix` |
| B17 — `edit` com substituições disjuntas | fechado | [`replace.rs`](../../../crates/nycode-agent/src/tools/edit/replace.rs) aplica da direita para a esquerda; [`edit.rs`](../../../crates/nycode-agent/src/tools/edit.rs) lê `replacements`. Testes `disjoint_replacements_edit_the_file_in_one_call`, `overlapping_replacements_are_refused`, `abutting_replacements_are_not_overlap` |
| B18 — `bash` com prazo por chamada | fechado | `timeout` no schema; [`Launch::with_deadline`](../../../crates/nycode-agent/src/tools/bash/launch.rs) troca só o prazo. Testes `a_per_call_timeout_overrides_the_construction_deadline`, `a_zero_timeout_is_refused_before_running` |
| B19 — `read` devolve imagem | fechado | reconhecimento por número mágico; [`ToolOutput::image`](../../../crates/nycode-agent/src/tool/mod.rs) e [`into_blocks`](../../../crates/nycode-agent/src/tool/mod.rs) no despacho. Testes `an_image_file_is_returned_as_an_image`, `an_image_result_becomes_a_tool_result_and_an_image_block` |
| B20 — teto de resultados em `grep`/`find`/`ls` | fechado | `limit` no schema; [`cap::of`](../../../crates/nycode-agent/src/tools/search/cap.rs) recorta no teto e recusa zero. Testes `a_per_call_limit_caps_below_the_default`, `a_limit_above_the_ceiling_is_clipped`, `a_zero_limit_is_refused` |
| B21 — `terminate` no resultado de ferramenta | fechado | `ToolOutput::terminate`; hook `Response.terminate` em `post-tool-use`; a rodada só para se **todas** as chamadas pedirem. Testes `a_terminating_result_skips_the_follow_up_model_turn`, `a_mixed_batch_does_not_stop_the_turn`, `a_post_tool_hook_stops_the_turn_without_denying` |
| B22 — direcionar vs enfileirar para depois | fechado | Enter durante o turno segue em `with_steering`; Alt+Enter só no `steer()` enfileira para o laço rodar depois. Fora do turno, Alt+Enter continua sendo quebra de linha. Testes `alt_enter_during_a_turn_queues_a_follow_up_instead_of_steering`, `a_queued_follow_up_runs_after_the_turn` |
| B23 — instruções ancestrais, do usuário e override | fechado | `from_sources` empilha config do usuário, ancestrais e raiz; `AGENTS.override.md` substitui AGENTS/CLAUDE/NYCODE só naquele diretório; o prompt exige `--trust-workspace-instructions`. Testes `an_ancestor_directory_contributes_its_instructions`, `user_config_instructions_are_loaded_before_the_project`, `an_override_in_a_directory_replaces_other_instruction_files_there`, `workspace_context_is_ignored_without_explicit_trust` |
...
| B26 — system prompt substituível e acrescentável | fechado | [`prompt.rs`](../../../crates/nycode-cli/src/invocation/prompt.rs); `--system` / `--append-system`; arquivos do workspace exigem `--trust-workspace-instructions`. Testes `a_project_file_is_not_allowed_to_replace_the_builtin`, `the_system_flag_replaces_files`, `a_trusted_append_file_follows_the_base`, `a_project_system_file_is_ignored_without_explicit_trust` |
| B27 — sessão nomeada, id escolhido, fork e import | fechado | [`open.rs`](../../../crates/nycode-cli/src/session/open.rs); `--name`, `--session-id`, `--fork`, `--import`. Testes `a_chosen_id_creates_the_session_when_it_is_missing`, `fork_copies_history_into_a_new_id`, `import_copies_a_jsonl_file_into_the_store`, `forking_a_cwd_file_without_records_is_not_picked_by_continue`, `fork_of_a_cwd_file_named_like_an_id_copies_the_file_not_the_store`, `a_path_like_id_is_refused_instead_of_leaving_the_store` |
| B28 — estatísticas, cópia, sessão nova, recarga | fechado | `/session` `/copy` `/new` `/reload` em [`builtin.rs`](../../../crates/nycode-cli/src/interactive/builtin.rs) e [`interactive.rs`](../../../crates/nycode-cli/src/interactive.rs). Testes `session_lists_id_name_and_message_count`, `copy_shows_the_last_assistant_text`, `a_new_session_drops_the_previous_history`, `reload_picks_up_a_command_written_after_startup`, `reload_keeps_the_invocation_system_flag` |
